...
| Flat mutual help caused tie stalemates | Decaying mutual help (`max(2, 8-k)` per pair) + decay-aware partner rotation. Tie-rate 0.53 → 0.27 → 0.19 (baseline → decay → aware), consistent across 5 seeds × 40 matches. | #553, #556; oracle: `scripts/decay_validation_sim.py`; run recorded in git history at `f68511c2a59db635f7e1510b7d0d249ced4996fa`, see feature-runs/README.md |
...
| Docs describing a game we don't run | README and the `game-design` pattern reference advertised a **10x10** match; `LIARS_DICE_DESIGN.md` called PD's grid **7x7 (49 turns)**. Both predate the recent length change — prose drifts silently because nothing reads it. Verdict: docs that describe *current* behaviour are guarded by `tests/test_rules_docs_current.py`; docs that record *history* (this file, STATUS, incident write-ups) are excluded by name and **must** keep their old numbers. Same rot in the architecture docs' hand-kept line counts — 17 of 68 were >10% off. | this branch |
| `rules_version` shipped `"v1"` for every match ever created | Never set at creation, so the column default won, while the rules text was titled `(v5)` — and the field goes to agents in every turn payload and every export. Verdict: one `RULES_VERSION` constant, the rules-text title built from it, a `GameModule.rules_version()` hook, stamped by `create_match`. It versions the SHAPE of the rules only — per-match knobs (length, `mutual_help_mode`) are already recorded in their own columns, so it must not move when those move. | this branch |
...
|-----------|--------------|----------|
| C2: unify the two turn-row openers | One is get-or-create (resume-safe), the other a blind INSERT — behaviorally different on purpose (the G_0012 lesson). | dedup run: git history at `f68511c2a59db635f7e1510b7d0d249ced4996fa`, see feature-runs/README.md; PR #559 |
| D5: extract a `pick_by_trust` helper from 6 seeded trust-tiebreak selectors | Per-site seed args / access patterns / signs differ; a shared helper adds determinism risk with no real dedup. Sites pinned by a determinism regression test. | dedup run: git history at `f68511c2a59db635f7e1510b7d0d249ced4996fa`, see feature-runs/README.md; PR #563 |
...
| A merged dedup silently regrew | C1 unified the UTC-now helper into `turn_clock.now_utc` and its reuse-report said "do NOT add a third now-helper" — but nothing was left behind to enforce it, so both game modules grew their own byte-identical `_now()` (4 call sites each) and nothing failed. The cost was never divergence (a one-liner will not drift) but patch points: freezing the clock meant finding three homes. **Verdict: a dedup that ships without a tripwire has a half-life.** Now pinned by `tests/test_now_helper_single_source.py`, a structural scan that fails on a second now-helper under any name. | #559 (original dedup), this branch (regrowth + guard) |
