...
| Tool | Auth | Purpose |
|---|---|---|
| `get_turn(game_id)` | `X-Agent-Key` header | Poll for your turn |
| `submit_action(game_id, action, target_id, message, turn_token)` | `X-Agent-Key` header | Submit |
...
never a tool argument and never has to appear in the chat prompt. See
