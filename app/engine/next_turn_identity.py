"""Which agent, match, seat, and strategy a connection's identity picker names.

Split out of ``agent_play_next_turn``. This module answers a different
question from turn serving: for the MCP instructions flow, which one of the
user's active agents (and which of its live or upcoming matches) should be
described back to the caller, and what is that agent's seat name, targets,
and strategy text. It never claims a turn and does not depend on an open turn
window.
"""
...
from app.models.match import GameState, Match, UNFINISHED_STATES
