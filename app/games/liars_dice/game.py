...
class LiarsDice(BaseGameModule):
    ...
    async def award_round(self, db: AsyncSession, game: Match, round_num: int) -> None:
        ...
        revealed: dict[str, list[int]] = {
            player.seat_name: _dice_list(player_states.get(player.id)) for player in players
        }
        challenger = _challenger_name(state_json)
        standing_bid_raw = state_json.get("standing_bid")
        bidder_raw = standing_bid_raw.get("by") if isinstance(standing_bid_raw, dict) else None
        bidder = bidder_raw if isinstance(bidder_raw, str) else None

        # Pure rules decide the showdown; this method only persists the result.
        outcome = resolve_showdown_outcome(
            standing,
            revealed=revealed,
            seat_order=_seat_order(players),
            bidder=bidder,
            challenger=challenger,
            elimination_order=state_json["elimination_order"],
            wild=config["wild_ones"],
        )

        loser_player = next(
            (player for player in players if player.seat_name == outcome.loser_seat), None
        )
        winner_player = next(
            (player for player in players if player.seat_name == outcome.winner_seat), None
        )

        if loser_player is not None and outcome.loser_new_count is not None:
            ...
            if loser_state is not None:
                ...
                if outcome.eliminated_seat is not None:
                    state_json["elimination_order"].append(outcome.eliminated_seat)
        ...
        state_json["last_showdown"] = {
            ...
            "bid_holds": outcome.bid_holds,
            "challenger": challenger,
            "winner": outcome.winner_seat,
            "loser": outcome.loser_seat,
            "revealed": {seat: list(dice) for seat, dice in revealed.items()},
        }
        ...
        state_json["next_leader"] = outcome.next_leader
