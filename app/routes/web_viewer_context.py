...
class _SeatLabels:
    """Public per-seat labels the standings rail and robot circle narrate by."""
...
@dataclass(frozen=True)
class _ViewerSeats:
    """The seat(s) this viewer holds, resolved for the play + coach panels."""

    viewer_human: Player | None
    viewer_agent: Player | None
    viewer_player: Player | None
    coach_player: Player | None


async def _load_seat_labels(db: DbSession, match_id: str) -> _SeatLabels:
    """Public labels for the standings rail + winner credit.

    The rail shows the agent name and the owner's byline; bots keep the platform
    credit. In a single match the seat name is the bot's identity: the turn feed,
    ...
    play-by-play, and two bots sharing a profile would collide. So show the seat
    name for bots here; humans/AI agents keep their full agent name.
    """
    owner_rows = (
        await db.execute(
    ...
    return _SeatLabels(owner_handles, agent_names, bot_flags, kind_by_seat)


def _build_scoreboard(players: list[Player], labels: _SeatLabels) -> list[dict[str, Any]]:
    """Rank the core standings, then decorate each row with the public labels
    and provider badge this page shows."""
    ...
    for row in rank_standings(
        ...
        seat = row["agent_id"]
        is_bot = labels.bot_flags.get(seat, False)
        provider = played_provider_by_seat.get(seat)
        scoreboard.append(
            {
                "agent_id": seat,
                "display_name": labels.agent_names.get(seat, seat),
                "round_score": row["round_score"],
                "round_wins": row["round_wins"],
                "owner_handle": labels.owner_handles.get(seat),
                "is_bot": is_bot,
...
def _resolve_viewer_seats(
    players: list[Player],
    user: User | None,
    kind_by_seat: dict[str, AgentKind],
) -> _ViewerSeats:
    """Resolve the viewer's human seat and agent seat separately.

    A user can hold two seats in one match since #478 (join as a human AND send
    an AI agent). The human seat you steer drives the play cockpit and the "you"
    highlight; the agent you sent drives the strategy/coach panels. The old
    single ``next(...)`` returned whichever seat sorted first by name, so a human
    who also sent an agent silently got the agent seat — and never saw their move
    controls.
    """
    ...
    return _ViewerSeats(viewer_human, viewer_agent, viewer_player, coach_player)


def _group_history_rounds(history: list[dict[str, Any]]) -> tuple[list[dict], int]:
    """Group the chronological turn history into rounds for the feed template.

    The template reverses it for the newest-first feed while round navigation can
    still reason about order.
    """
    rounds: list[dict] = []
    for h in history:
    ...
    return rounds, max_played_round


async def _resolve_winner_seat(db: DbSession, match: Match) -> str | None:
    """The winning seat name for the winner-credit line, or None."""
    if not match.winner_player_id:
        return None
    winner = (
        await db.execute(select(Player).where(Player.id == match.winner_player_id))
    ).scalar_one_or_none()
    return winner.seat_name if winner else None


async def _load_coach_prompt(
    db: DbSession, coach_player: Player | None
) -> tuple[str | None, str | None]:
    """The strategy text + version label for the coach modal, or (None, None)."""
    if coach_player is None or coach_player.agent_version_id is None:
        return None, None
    version = await _load_viewer_prompt_version(db, coach_player.agent_version_id)
    if version is None:
        return None, None
    return version.strategy_text, f"v{version.version_no}"


def _countdown_start_iso(match: Match) -> str | None:
    ...
    """
    if (
        match.state in (GameState.SCHEDULED, GameState.REGISTERING)
        and match.match_kind != MatchKind.PRACTICE_ARENA.value
    ):
        return ensure_aware(match.scheduled_start).isoformat()
    return None


def _order_play_talk(
    talk: list[dict[str, Any]], score_by_name: dict[str, Any]
) -> list[dict[str, Any]]:
    """Order this turn's revealed talk by round score (highest first; silent
    last), so the live reveal reads in standings order (spec 019)."""
...
def _apply_player_mode_keys(
    ctx: dict[str, Any],
    play_ctx: dict[str, Any],
    scoreboard: list[dict[str, Any]],
    viewer_seat: str | None,
    match: Match,
) -> None:
    """Player-mode cockpit (spec 018). When the seated viewer is a HUMAN in a
    LIVE match the page becomes a play cockpit: the input docks at the bottom,
    feed names carry score chips, and the animated replay steps aside (it returns
    as the lead once the game is completed). These keys also feed the /live
    ...
    if talk:
        ctx["play_talk"] = _order_play_talk(talk, score_by_name)
    player_mode = bool(play_ctx.get("viewer_is_human")) and match.state == GameState.ACTIVE
...
async def _game_view_context(request: Request, db, match: Match) -> dict:
    ...
    labels = await _load_seat_labels(db, g.id)
    scoreboard = _build_scoreboard(players, labels)

    seats = _resolve_viewer_seats(players, user, labels.kind_by_seat)
    viewer_player = seats.viewer_player
    coach_player = seats.coach_player
    ...
    play_ctx = await _build_human_play_context(
        db, g, players, seats.viewer_human, labels.kind_by_seat, history
    )
    ...
    rounds, max_played_round = _group_history_rounds(history)
    winner_agent_id = await _resolve_winner_seat(db, g)
    viewer_prompt_text, viewer_prompt_label = await _load_coach_prompt(db, coach_player)
    countdown_start_iso = _countdown_start_iso(g)
    ctx = {
        "user": user,
        ...
        "winner_owner_handle": labels.owner_handles.get(winner_agent_id) if winner_agent_id else None,
        # The coach panel (strategy link + note) is about the agent you sent.
        "viewer_player_id": coach_player.id if coach_player else None,
        "viewer_agent_name": labels.agent_names.get(viewer_player.seat_name) if viewer_player else None,
    ...
    _apply_player_mode_keys(ctx, play_ctx, scoreboard, viewer_seat, g)
    return ctx
...
async def _latest_open_turn(db: DbSession, match_id: str) -> Turn | None:
    """The most recent unresolved turn for this match, or None.

    Ordered by ``id`` alone — the viewer only ever has one open turn, and this
    keeps the well-worn viewer query shape intact.
    """
    return (
        (
            await db.execute(
...
async def _load_acted_player_ids(db: DbSession, turn: Turn) -> set[int]:
    """Player ids that have really acted this phase (defaulted moves don't count).

    Talk phase reads ``TurnMessage``; act phase reads ``TurnSubmission``.
    """
    if turn.phase == "talk":
        stmt = select(TurnMessage.player_id).where(
            TurnMessage.turn_id == turn.id,
            TurnMessage.was_defaulted.is_(False),
        )
    else:
        stmt = select(TurnSubmission.player_id).where(
...
async def _load_submitted_talk_text(db: DbSession, turn: Turn, player: Player) -> str:
    """The viewer's already-sent message for this open talk turn, or ``""``."""
    return (
        await db.execute(
...
async def _load_submitted_move(
    db: DbSession, turn: Turn, player: Player, players: list[Player]
) -> tuple[str | None, str | None]:
    """The viewer's own (action, target seat) for this open act turn.

    Returns ``(None, None)`` when the viewer hasn't submitted, and a ``None``
    target when the action has no target.
    """
    sub = (
        await db.execute(
    ...
    if sub.target_player_id is not None:
        target = next((p for p in players if p.id == sub.target_player_id), None)
        target_seat = target.seat_name if target else None
    return sub.action, target_seat


async def _viewer_move_state(
    db: DbSession,
    turn: Turn,
    viewer_player: Player,
    active: list[Player],
    ...
    """The turn-specific move keys for a seated human once a turn is open."""
    phase = turn.phase
    submitted = viewer_player.id in acted_ids
    state: dict[str, Any] = {
        "play_submitted": submitted,
        "can_play": viewer_player.autopilot_at is None,
        "play_targets": [p.seat_name for p in active if p.id != viewer_player.id],
    }
    if phase == "talk" and history:
        # Recap the turn that just ended so the human reads the outcome (who did
        # what, for how many points) before speaking again. This is the
        # talk-phase mirror of the act-phase talk reveal below. The open talk turn
        # isn't in `history` (it lists resolved turns only), so the last entry is
        # the turn that just resolved.
        last = history[-1]
        state["play_last_result"] = last["feed_actions"]
        state["play_last_label"] = f"Round {last['round']} · Turn {last['turn']}"
    if phase == "talk" and submitted:
        # Keep the human's already-sent message in the box so the panel reflects
        # what they said (the act-phase mirror of re-checking the chosen action
        # below). Without this the input re-renders empty after submit, which
        # reads as "nothing was sent" even though it was.
        state["play_message"] = await _load_submitted_talk_text(db, turn, viewer_player)
    if phase == "act":
        # Reveal this turn's talk so the human reads what was said before acting —
        # the same transcript the bots get in their act-phase prompt. This open
        # turn isn't in the feed yet (the feed only shows resolved turns), so
        # without this the human would act blind.
        state["play_talk"] = await _build_turn_talk(db, turn, players, viewer_player)
    if phase == "act" and submitted:
        action, target = await _load_submitted_move(db, turn, viewer_player, players)
        state["play_action"] = action
        state["play_target"] = target
    return state
...
async def _build_human_play_context(
    ...
    turn = await _latest_open_turn(db, match.id)
    if turn is None:
    ...
    if viewer_player is not None and viewer_is_human:
        ...
        base.update(
            await _viewer_move_state(
                db, turn, viewer_player, active, acted_ids, players, history
            )
        )
    return base
