...
async def _build_agent_detail_context(
    ...
    # One shared readiness→badge mapping, also used by the agent list page
    # (app/routes/agents_list.py), so the two pages can't disagree about whether
    # the same agent is ready. health_view turns it into the plain dict the
    # templates read (same keys as ConnectionHealthStatus, not the dataclass).
