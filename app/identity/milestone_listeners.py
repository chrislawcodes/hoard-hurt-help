...
def _on_user_insert(_mapper: Any, _connection: Any, target: User) -> None:
    # The platform's own bots account is not a signup.
    if target.google_sub == BOTS_USER_SUB:
        return
...
def _on_player_insert(_mapper: Any, _connection: Any, target: Player) -> None:
    """Collect separately: player rows need the bots-user filter applied later.

    Platform bots are seated as real Player rows owned by the bots account, so
    without this filter every bot in every match would read as that account
    joining a match.
    """
    session = object_session(target)
    if session is None:
...
def _drop_bots_user_rows(
    session: Session, rows: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    """Remove player milestones belonging to the platform bots account."""
    if not rows:
        return rows
    user_ids = {row["user_id"] for row in rows}
    bots_user_id = session.scalar(
        select(User.id).where(User.google_sub == BOTS_USER_SUB, User.id.in_(user_ids))
    )
    if bots_user_id is None:
        return rows
    return [row for row in rows if row["user_id"] != bots_user_id]
...
def _on_after_flush_postexec(session: Session, _flush_context: Any) -> None:
    ...
    record_pending_sync(session, rows + _drop_bots_user_rows(session, player_rows))
