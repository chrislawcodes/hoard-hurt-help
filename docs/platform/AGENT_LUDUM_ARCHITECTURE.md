...
| Choose / resolve a per‑agent model | The optional `Agent.preferred_model` (`app/models/agent.py`) — picker + effective‑model + verification status on the agent‑settings page (`app/routes/agents_detail.py` / `agents_lifecycle.py` + `app/templates/agents/`); resolution is `resolve_seat_model` in `app/engine/model_provider_match.py`, called from `app/engine/agent_play_next_turn._build_turn_payload`. **One control, one record:** that resolved answer is stamped onto `Player.played_model` when the seat is first claimed (beside `played_provider`, migration `0056`), and the export reads **that stamp**, never the agent's live setting — editing an agent must not rewrite what a past match played. NULL means "not recorded" (seats predating the column, or never served) and is deliberately not backfilled from today's settings. The join/seat page stays provider‑only (do **not** add a model picker to `web_join.py`). |
...
|---|---:|---|
| `web.py` | 24 | Aggregates the split human web routers below so `app.main` still mounts one router. |
| `web_lobby.py` | 349 | The lobby board itself (`/games/{game}` + the polled `upcoming` fragment) **and the aggregated router** that splices in the lobby‑area siblings below in their original registration order. (Was a 639‑line catch‑all; split by page area.) |
| `web_front_page.py` | 68 | Agent Ludum marketing front page (`GET /`). |
...
| `match_bots_web.py` | 177 | Bot seating for a match (was `game_admin_bots_web.py`). Prefix `/games/{game}/admin`. **Owner or platform admin** via `OwnedOrAdminMatch`. Reached from the "Manage →" link on `/me/matches`. |
| `admin_match_actions.py` | 136 | **Shared body** for the admin match APIs (was `game_admin_actions.py`): create‑record, cancel, CSV/JSON export, and match‑load helpers. `admin_api.py` and `match_export_api.py` are thin wrappers over it. |
| `match_export_api.py` | 51 | Game‑scoped match **exports**, open to **any signed‑in user** (was `game_admin_api.py`). Prefix `/api/game-admin/{game}` — a legacy URL kept for compatibility; it no longer implies any admin gate. Its create and cancel routes were **deleted**: exact duplicates of `/api/admin/*`, and a second player‑reachable create route would have bypassed the 3‑match cap. |
| `admin_api.py` | 76 | **Platform admin** JSON: create/cancel matches, export data. Guarded by `require_platform_admin`. Thin wrappers over the shared `admin_match_actions.py` bodies; passes an admin `ExportViewer` so its export stays unredacted. |
| `handle_web.py` | 169 | Public **handle** pick/change pages (the one‑time "choose your @handle" gate that `require_user_with_handle` enforces). |
...
| `spectator_api.py` | 117 | Public spectator JSON. **Never** returns strategy prompts, and reads resolved turns only. The match export (`app/read_models/match_export.py`) applies the same two rules to a non‑admin caller. |
...
| Module | Lines | Responsibility |
|---|---:|---|
| `scheduler.py` | 465 | **Registry + due‑game poller.** Tracks the running asyncio task per active game; auto‑starts and cancels due games; resumes task loops after a process restart. The poller runs seven subsystems per tick (arena fill/create, held‑seat sweep, start‑due, the watchdog, then the overdue‑turn sweeper); each is failure‑isolated and escalates to CRITICAL after repeated failures. The per‑match turn‑loop logic lives in `scheduler_turn_loop.py` and is re‑exported here so callers and tests keep the same import path. |
| `scheduler_turn_loop.py` | 423 | **Per‑match turn loop.** Owns `_run_game`, `_open_turn`, and the `_wait_for_*` helpers — split from `scheduler.py` to isolate the freeze‑prone resume path. Re‑exported through `scheduler.py`; the dependency is one‑directional (scheduler imports turn loop, never the reverse). |
...
| `connection_health.py` | 112 | **Thin aggregator** — re‑exports the three modules below so callers keep one import path. The connection‑health logic was split out by job; this file owns no logic of its own now. |
...
| `provider_readiness.py` | 419 | The single **per‑provider** readiness signal `ProviderReadiness` (`NO_MCP_CONNECTION` / `CONNECTED_NOT_LIVE` / `SEEN_NOT_POLLING` / `LIVE`) + `provider_readiness()` — a thin wrapper over the `provider_has_current_setup` / `provider_has_live_current_setup` / `provider_loop_running` predicates (also here; adds no new query) + `enabled_provider_values`. This is the **one** answer to "is this provider set up / connected / playing" that the play‑setup gate and every readiness badge read, instead of each site picking its own predicate. |
...
| `arena.py` | 398 | Managed Practice Arena and Auto‑Match creation: idempotent poller helpers, shared Bot seeding, and start timing. **Both seat 7 players** — Practice Arena = `PRACTICE_ARENA_MAX_PLAYERS` (6 pre‑seeded bots + 1 open human seat); Auto‑Match = `AUTO_MATCH_MAX_PLAYERS` (the external agent that triggers the start + bots filling the rest). **Auto‑Match opens one match per 15‑minute clock boundary** (`AUTO_MATCH_INTERVAL_MINUTES`, dropped from 30 in #464). |
| `agent_idle.py` | 299 | **Server‑side poll pacing for `get_next_turn`.** `pace_idle` decides, off the *soonest* game the caller is seated in, how the next poll behaves so an interactive AI "asks as rarely as possible without missing a turn" (every ask is a paid model think). In a live game it **long‑polls** — holds the request open (cheap; no model thinking) and answers the instant a turn opens (single DB session per hold, ~5s internal check — #462). Before a game it returns a paced `next_poll_after_seconds` (~5 min far out → ~1 min in the last five → long‑poll in the final minute). Also owns `should_stop` (only fires when there is **no** game at all and the idle clock passes `IDLE_STOP_SECONDS`; the always‑on connector ignores it). |
| `resolver.py` | 155 | **Generic turn‑lifecycle helpers only:** `finalize_talk_phase`, `award_round_winners`, `finalize_game`. Fully game‑agnostic. PD‑specific per‑turn scoring (HOARD/HELP/HURT payoffs, mutual‑help bonus, score floor) moved to `app/games/hoard_hurt_help/scoring.py`. |
...
| `turn_drivers.py` | 212 | **Per‑game‑shape turn drivers behind one interface** — how the scheduler advances a turn for a simultaneous game (PD) vs. a sequential one (Liar's Dice), so the loop in `scheduler_turn_loop.py` stays game‑shape‑agnostic. |
| `win_probability.py` | 417 | Win‑probability predictions from pre‑trained scikit‑learn models (`score_round_win`). **Not currently wired into any UI** — the PD replay's win‑probability overlay was removed and its glue (`viewer_win_probs.py`) deleted; the engine, the trained `.pkl` models, and the training scripts remain on disk, dormant. |
| `agent_onboarding.py` | 192 | Onboarding‑state resolution for AI agents (`AgentOnboardingState` — in‑game progress, distinct from the connection badge and provider readiness). |
...
| Module | Lines | Responsibility |
|---|---:|---|
| `strategies.py` | 282 | The **9** personalities (incl. `pragmatist`, which betrays its helper at the buzzer): pick a talk intent, then an action intent, from public state. Known traitors (trust ≤ `HOSTILE_TRUST`) are frozen out of fresh cooperation offers. |
| `service.py` | 242 | DB‑facing glue: the scheduler calls this each phase to auto‑submit every Bot's talk/action. |
| `runtime.py` | 303 | Orchestration: build a Bot's profile, run the talk/action decision. |
| `trust.py` | 283 | Per‑Bot trust scoring from resolved actions + talk signals, with **betrayal memory** (remembers who HURT a helper; the sting fades over each model's `forgive_rounds`) and **partner fatigue** (`PARTNER_FATIGUE` — a farmed mutual‑help pact's trust decays toward 0 per prior mutual‑help turn, mirroring the scoring‑side decay so bots rotate off stale allies). |
...
| Module | Lines | Responsibility |
|---|---:|---|
| `request_logging.py` | 242 | Global request logging, incident capture, 500 handling. |
...
| `main.py` | 347 | App factory, lifespan (migrate → resume → poll), router mounting. Lifespan also logs a loud startup warning when `platform_admin_emails_set` is empty (advisory only — an empty bootstrap list removes the immutable admin floor; does not block boot). |
