...
| Change sideline coaching (the "Coach" note an owner sends their agent) | `app/routes/web_viewer.py` (`POST .../coach-note` + the `coach_panel.html` fragment, triggered by the **"Coach" button in the standings rail** since #465) writes `player.coach_note` / `coach_note_round`; `app/engine/agent_play_reads.py` (`build_turn_static_dict`) injects it as `static.coach_note` for the round it targets — the one builder both turn-serving paths (the next-turn fan-out AND the per-match poll) emit their static block from, so a change there reaches both; the MCP loop honors it via `_mcp_how_to_play_block`. Columns live on `app/models/player.py`. |
