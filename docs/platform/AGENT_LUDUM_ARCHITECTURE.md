...
| `web_viewer_context.py` | 598 | Context assembly for the viewer page and its live fragment (split out of `web_viewer.py`): `_game_view_context` builds the generic skeleton (players, scoreboard, timeline, messages), resolves the viewer's seat(s) separately — a human seat and a sent AI-agent seat can both belong to the same user, so each drives its own panel instead of one silently shadowing the other — and merges in each game module's `build_replay_view` payload (PD's builder: `app/games/hoard_hurt_help/viewer.py`; Liar's Dice: `app/games/liars_dice/viewer.py`); `_build_human_play_context` builds the **human play-panel context** (open turn, phase, deadline, submitted state, target list, this-turn's talk for the act phase, the everyone-visible "waiting on N" count) + the join/leave CTA flags; the **player-mode cockpit** keys (leader seat, per-seat scores, the standings-ordered talk reveal) are computed once and shared by both the full page and the `/live` SSE fragment so they can't drift out of sync across a swap. |
...
analysis pages do not each rebuild the same DB shape by hand. `version_stats.py`
projects each `AgentVersion`'s completed-match record (rated wins, practice
count) and its most recent match links, batched over many version ids at once —
shared by the agent detail page's version timeline and the join screen's agent
cards, so both surfaces show the same record for a version. `lobby_onboarding.py`
answers the lobby's one onboarding-banner question — does this user have a warm,
connected agent and no game to play — checked cheapest-first so the page never
pays for a per-connection health computation on a user who can't see the banner
anyway.
...
### Presentation — `app/templates/` (63 files, ~4,385 lines) + `app/static/style.css` (~2,820) + the robot‑circle viewer's own static assets (~1,810 lines)

Server‑rendered Jinja with a fixed platform shell (`base.html`) and HTMX
fragments (`templates/fragments/`) swapped in over SSE. One platform stylesheet
(`style.css`) covers the site; a game tints only its content region via scoped
CSS variables. The robot‑circle viewer's own JS/CSS moved out of templates into
static files it links instead of inlines — `robot-circle.css`, `rc-replay.js`,
`rc-countdown.js`, `round-nav.js` (plus the pre‑existing `rc-lobby-banter.js`)
— so `fragments/robot_circle.html` is now a thin shell of `<link>`/`<script
src>` tags around the markup include, cacheable by the browser instead of
re‑rendered into every page.
