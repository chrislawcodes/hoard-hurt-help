...
| `web_support.py` | 542 | Shared web helpers for match URLs, legacy redirects, player counts, game themes, upcoming cards, and standings — now also the bulk of the file: seat‑name allocation (`unique_seat_name`), the `safe_internal_next` open‑redirect guard, the admin/visibility auth predicates (`_is_any_admin` / `_is_game_admin` / `_can_view_game`), and the game‑slug‑redirect + match‑loading dependency machinery (`GameSlugRedirect`, `game_slug_redirect_response`, `raise_for_game_slug_mismatch`, the `_make_game_scoped_match_loader` builder + the `GameScopedMatch*` dependency singletons, and the shared match‑by‑id‑or‑404 loader). |
