...
| Change connection health / liveness | `app/engine/connection_health_badge.py` (the `ConnectionHealth` enum + `compute_connection_health`; reads `last_seen_at`/`runner_pid` + `players.served_by_connection_id`, not agent attachment). |
...
| `connection_health.py` | gone | **Deleted (PR C).** It was a thin aggregator re‑exporting the three modules below; callers now import each symbol directly from whichever of `connection_health_badge.py`, `provider_readiness.py`, or `join_gate_capacity.py` defines it. |
