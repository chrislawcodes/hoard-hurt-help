...
| `spectator_api.py` | 118 | Public spectator JSON. **Never** returns strategy prompts, and reads resolved turns only. The match export (`app/read_models/match_export.py`) applies the same two rules to a non‑admin caller. |
