...
The original plan listed three paths (an MCP server, a ChatGPT Custom GPT, and a public OpenAPI spec). What shipped is **MCP-centric** — the Custom GPT and public-OpenAPI paths were not built:
...
The counts below belong to the game module, not the platform, so they are
generated rather than restated here — this section described a 10x10 match for a
long time after the game stopped being one:

<!-- BEGIN GENERATED: shipped-match-shape -->
<!-- Generated by scripts/render_generated_docs.py from the code. Do not edit by hand — change the constants and re-run. -->

The shipped Hoard-Hurt-Help match is **7 rounds of 5 turns** (35 turns total), **6-10 players**, on rules **v11**. Every count is admin-configurable per match; these are what `config_defaults()` returns.
<!-- END GENERATED: shipped-match-shape -->

**Static prefix — sent at the top of every payload, identical across all turns of a match:**
- Full game rules text (with version)
- Match ID
- Total rounds and total turns per round for this match
...
3. The join screen is a **lineup**: one row per entrant, all visible at once, because the decision ("who of mine is entering, and on which AI") is usually several picks made against a running countdown. The choices are **independent** — a player can take a **human seat**, **send AI agents**, or do **both** in the same match (play by hand *and* field your own bot, so you compete against it):
   - **Play manually** is pinned first and never moves. It needs only a display name (pre-filled from the handle). No agent, no connection, no key — so a brand-new visitor's first line is a working option.
   - **Each agent** is one row: name, the owner's short description (`Agent.blurb`), and its AI pills. The pills appear only once the row is ticked, and ticking auto-selects the first free AI, so a row is never half-answered. An AI's state shows only when there IS news (idle / not connected / busy) — a ready AI carries no badge, because a column of identical "ready" labels tells the reader nothing. "One AI = one seat at a time" greys out any AI already committed, on this page or another unfinished seat.
...
separate authentication of rules content or strategy prompt is needed — the server
