...
`discover` also records a third question that doesn't change the path recommendation above but does change review depth: `--completeness-risk yes|no "<note>"` — does one changed value thread through multiple consumers or render paths (server code, templates, client scripts, payloads, docs)? When `--completeness-risk yes`, add the `completeness-adversarial` lens at the plan checkpoint and at the diff checkpoint (on a one-shot run, its single diff checkpoint), via `--extra-gemini-lens completeness-adversarial` or the `prepare-claude-reviews` equivalent — see "Size-gated diff review" below.
...
> Update the scoped architecture doc to describe the system **as it will be after this feature**: new or changed modules (with their responsibility and rough size), new runtime flows, and the "Where to make a change" index. Update the scoped design doc only if this feature changes a game/design decision. Keep edits surgical and consistent with `plan.md` and `reuse-report.md`. Do not invent modules the plan doesn't call for.

These edits live on the feature branch and ship with the code, so by merge time the docs match reality. **They are reviewed by Codex at the plan checkpoint** — pass them in with `--context <scoped architecture doc> --context <scoped design doc>`. Treat doc-accuracy and duplication findings like any other plan finding: reconcile before advancing.
...
**Spec-review rounds under a settled design.** When `discover` recorded `--design-settled yes`, run one spec-review round by default — reconcile the round-1 findings to a terminal status and proceed to Design/Plan. Open a second round only if a High/Critical-severity finding is still open afterward; a design that was already settled going in doesn't need a second pass to re-litigate what round 1 already confirmed. The `betrayal-8-4` run spent two spec rounds re-reviewing a design both arms already had fully settled up front — pure cost (`docs/workflow/experiments-thin-vs-factory.md`, Run 2). This is orchestrator judgment, not a new engine flag: `MAX_ADVERSARIAL_ROUNDS` and `--accept-cap` above still apply as the hard ceiling.
...
On a **one-shot** run (see "Keep Diffs Scoped") the single diff checkpoint covers the whole feature — it is the run's main defense, and a whole-feature diff exceeds the threshold in practice, so the review always fires. Widen it rather than splitting the build: when discovery recorded `--completeness-risk yes` (see "Routing: two questions decide the path" above), add the `completeness-adversarial` lens via the checkpoint's extra-lens option (`--extra-gemini-lens completeness-adversarial`) or an extra Claude lens through `prepare-claude-reviews`. The focused-review value that slicing was buying comes from this review fan instead, not from slice boundaries: `completeness-adversarial` is the class of review that caught the `betrayal-8-4` run's decisive gap — a changed value missing from two render paths — at the plan checkpoint.
...
| scoped design/architecture docs (`docs/platform/…`, `docs/games/<game>/…`) | **Living system docs** — updated up front at the plan stage (reviewed by Codex) and reconciled at closeout if implementation drifted. Not per-run artifacts; they are the durable, always-current view of the codebase. |
...
- For workflow-system improvement work, treat `docs/workflow/plans/feature-workflow-plan.md` (if present) as the maintained plan of record and keep it in sync with this skill.
...
## Preflight Gate (hoard-hurt-help)

This repo's "run build and tests" step at every implementation slice and diff checkpoint means the Preflight Gate from `CLAUDE.md`, run from the repo root:
