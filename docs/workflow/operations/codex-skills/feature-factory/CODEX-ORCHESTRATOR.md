...
| **Discovery** | **Mandatory before spec.** Ask clarifying questions one at a time (or state assumptions), then fill the **required checklist**: goal, audience, success criteria, non-goals, constraints/risks, plus the two routing answers (silent-risk, design-settled — each `yes`/`no` with a short note) and the review-depth answer `--completeness-risk yes|no "<note>"` (does one changed value thread through multiple consumers/render paths? adds the `completeness-adversarial` lens at plan/diff checkpoints when `yes`). `discover --complete` refuses until all seven required-checklist items are recorded (use `--force-complete` only for trivial / skip-FF features) and prints the routing recommendation (see "Routing: two questions decide the path" in SKILL.md). | `discover --slug <slug> --question "..." --recommendation "..." --rationale "..."` (repeat), then `discover --slug <slug> --goal "..." --audience "..." --acceptance-criteria "..." --non-goal "..." --constraints "..." --silent-risk yes "<note>" --design-settled no "<note>" --completeness-risk no "<note>" --summary "..." --complete` |
...
| **Design · Reuse audit** | Dispatch a read-only `codex exec` sub-session: scan the codebase (guided by the scoped architecture doc) for modules overlapping the feature; write `reuse-report.md` (capability → existing module → reuse/extend/justified-new). The plan must address every row. | `codex exec -m gpt-5.4-mini "<reuse-audit prompt>"` → write `docs/workflow/feature-runs/<slug>/reuse-report.md` |
| **Design · Update scoped docs** | Dispatch a workspace-write `codex exec` sub-session: update the scoped architecture doc (and the scoped design doc if a design decision changes) to the **target state** (the design, before the build plan), consistent with the plan and reuse report | `codex exec -m gpt-5.4-mini -s workspace-write "<doc-update prompt>"` |
...
| **Plan checkpoint** | Generate adversarial reviews, judge findings — **include the doc edits + reuse report in the review** | `checkpoint --slug <slug> --stage plan --context docs/platform/AGENT_LUDUM_ARCHITECTURE.md --context docs/games/hoard-hurt-help/HOARD_HURT_HELP_ARCHITECTURE.md --context docs/workflow/feature-runs/<slug>/reuse-report.md` |
| **Write tasks** | Decide slice count first — **one-shot (a single trailing `[CHECKPOINT]`) is the default**; slice only for ordered steps, >~300 changed lines, or data-critical gates. Slices must be vertical and green alone (see "Keep Diffs Scoped" in SKILL.md). Record the decision + reason at the top of `tasks.md`. | Write to `docs/workflow/feature-runs/<slug>/tasks.md`, then checkpoint |
