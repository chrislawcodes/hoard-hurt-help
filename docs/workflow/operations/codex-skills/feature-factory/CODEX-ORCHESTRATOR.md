...
| **Update docs (up front)** | Dispatch a workspace-write `codex exec` sub-session: update the scoped architecture doc (and the scoped design doc if a design decision changes) to the planned target state, consistent with the plan and reuse report | `codex exec -m gpt-5.4-mini -s workspace-write "<doc-update prompt>"` |
...
**Hard gates:** `checkpoint --stage plan` refuses to run without `reuse-report.md`, and `status` will not return `done` until the scoped design/architecture docs changed since init or a no-change ack is recorded.
