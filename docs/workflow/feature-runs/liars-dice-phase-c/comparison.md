...
> **Historical record, filed late.** This run's artifacts lived only on the
> `factory/liars-dice-engine` branch and never reached `main`, so this folder was
> missing while every other run in `docs/workflow/feature-runs/` had one. It is
> copied here unchanged; read the body as what was true *during the experiment*.
>
> **What changed after it was written:** PR #377 is described below as "open,
> intentionally not merged". It was closed on 2026-06-14, superseded by **#388**,
> which shipped Phase C on `main`'s corrected engine while reusing this arm's
> module / bots / viewer / admin code. Liar's Dice is live today — see
> `app/games/liars_dice/`. The headline findings are also summarised as run 10 in
