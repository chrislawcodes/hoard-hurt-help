...
- **FR-011**: Editing an agent's model or strategy MUST update the current version **if it has not yet played a rated match**, and MUST otherwise create a new version (N+1) that becomes current. A version becomes **frozen** (immutable) at the **moment its first rated match starts** (when it is first seated into a starting match), recorded by `frozen_at`; editing MUST be blocked while a version is mid-match. Rating MUST be computed **per version**. *(Practice/unrated matches do not freeze a version.)* Supports US5.
...
- **FR-024**: Connection **health** (live / stalled / ready) MUST be computed at the connection level across its agents (heartbeat, `stall_threshold`, `max_concurrent_games`, paused state) — not via single-agent assumptions. The combined-create flow MUST handle a connection that never connects: a `pending` connection is created on provider-select, can be resumed if abandoned, and is garbage-collected after 24h. The agent is only created **after** its connection first goes active, so a `pending` (never-connected) connection has **no** agents — GC of a pending connection therefore cannot orphan an agent. (Detach, FR-029, applies only to deleting an *active* connection that powers agents.) Supports US1, US6.
...
- **FR-015**: Connection management MUST support reissue (graceful overlap — old key valid until new one connects), revoke (immediate cutoff), pause/resume, and delete. Deleting a connection that still powers agents MUST **detach** those agents per FR-029 (they survive, paused, reattachable) — NOT block. Supports US6.
