...
`[CHECKPOINT]` CP1 — ruff+mypy on changed files; test_connection_management.py green.
...
`[CHECKPOINT]` CP2 — ruff+mypy on changed files; test_agent_versions.py green.
...
`[CHECKPOINT]` CP3 — app imports; ruff+mypy on changed files.
...
`[CHECKPOINT]` CP4 — ruff+mypy on changed files; runner test green.
...
`[CHECKPOINT]` CP5 — full preflight green; ready for deliver.
