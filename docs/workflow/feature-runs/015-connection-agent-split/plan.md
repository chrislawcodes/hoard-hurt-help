# Implementation Plan — 015 Connection/Agent Split (FF pick-up, slices 3–5)

Slices 0–2 are committed (models+schema, connection auth + (agent,match) turn
resolution, bots-as-agents + leaderboard). This plan covers the REMAINING work.
Reuse decisions: see `reuse-report.md` (everything renames/extends an existing
`bots_*`/auth/runner module — no parallel systems). Architecture target state is
already written into `ARCHITECTURE.md` + `DESIGN.md` §12.

Remaining slices:
- Slice 3 — management UI: split `bots_*` routes → `connections_*` (/me/connections)
  + `agents_*` (/me/agents, /me/agents/new); agent versioning (create v1, fork on
  edit, freeze at first rated-match start); connection_health across a connection's
  agents; max_concurrent enforcement at join; detach-on-connection-delete + reattach;
  pending-connection flow; templates split connections/+agents/; migrate
  web_player/web_viewer/web_support/admin_web/nav_context/auth.
- Slice 4 — runner rename `agentludum_agent.py`→`agentludum_connector.py`; mcp_server
  X-Connection-Key naming; drop the MCP-direct connect path.
- Slice 5 — rename sweep (no Bot class, no /me/bots, never "bot" for a user's player);
  full preflight green (ruff + mypy app/ mcp_server/ + pytest -q), no suppressions.

Risk + verification:
- HIGH-CARE already landed (slice 1 turn resolution). Residual risk for this plan:
  the route restructure could break imports app-wide. verification: `python3 -c
  "import app.main"` must succeed and `pytest -q` green at slice 5.
- Detach semantics: verification — a test deletes a connection powering an agent and
  asserts the agent survives detached (paused, reattachable), FR-029.
- Versioning: verification — a test edits a played agent's strategy and asserts a new
  AgentVersion (N+1) is forked and the completed match still resolves its old version.

The full reviewed plan (decisions, file scope, sequencing) follows.
...
- review: reviews/plan.gemini.testability-adversarial.review.md | status: accepted | note: All 3 HIGH findings already addressed by committed slices 0-1: (1) agent_turn_token replay bounded by existing turn idempotency (DB unique(turn,player) + existing-check + turn resolution; slice-1 fanout test proves wrong-agent submit rejected); (2) migration 0023 downgrade implemented, round-trip test passes; (3) editing blocked mid-match (FR-011) + player.agent_version_id pins each match's version. Become explicit tests in the tasks stage; import-restructure risk covered by slice-5 gate (import app.main + pytest green).
- review: reviews/plan.codex.implementation-adversarial.review.md | status: deferred | note: Codex review runner timed out (infra; no content) — recurring this session. Plan was Codex+Gemini reviewed in Round 1 (review-log.md); Gemini's engine review this round had no plan-blockers. Defer the feasibility/implementation check to the per-slice diff checkpoints where the actual code is reviewed by Codex+Gemini.
- review: reviews/spec.gemini.requirements-adversarial.review.md | status: accepted | note: All findings fixed in commit f8927b5: HIGH FR-015/FR-029 contradiction (delete=detach not block) resolved; version-freeze timing pinned to first rated-match start (FR-011); pending-connection-orphan clarified (agent created only after connect, so GC can't orphan, FR-024).
- review: reviews/spec.codex.feasibility-adversarial.review.md | status: accepted | note: Spec re-checkpoint completed with no actionable findings ('reviews ran, no actionable findings raised'). Spec also adversarially reviewed in Round 1 (review-log.md).
