Review this plan artifact using a testability-adversarial lens.
...
| `_has_moved` (C5) | `app/engine/connection_activity.py:80` == `app/engine/agent_onboarding.py:127` | reuse | Byte-identical; keep one. Home must avoid a cycle between the two onboarding modules — see plan. |
