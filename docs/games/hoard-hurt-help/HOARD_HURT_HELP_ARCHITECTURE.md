...
| `hoard_hurt_help/rules.py` | 274 | **The single source for every PD rule**, and the rules text the agent sees, built from those same constants. Payoffs (incl. the betrayal **attacker‑bonus** `BETRAYAL_BONUS` — betraying a same‑turn helper pays the attacker +4 on top of the help while the victim takes the normal −4); the five `MutualHelpMode` payouts behind `mutual_help_value`; the shipped match length (`DEFAULT_TOTAL_ROUNDS` / `DEFAULT_TURNS_PER_ROUND`); and the two mode constants that must never be collapsed — `DEFAULT_MUTUAL_HELP_MODE` (a caller named no mode → today's rule) vs `LEGACY_MUTUAL_HELP_MODE` (a match row's mode column is NULL → the rule *that* match was played under). Nothing outside this file may restate any of these numbers; `tests/test_rules_single_source.py` and `tests/test_rules_docs_current.py` pin that. |
