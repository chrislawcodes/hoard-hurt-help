...
`_seed_*` helpers are duplicated from there (both halves need them); the
`reset_db` fixture itself is conftest's shared one — every test here already
requests it by name, so no local autouse copy or alias is needed.
"""
