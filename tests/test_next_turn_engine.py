...
def _c(game_id: str, deadline_offset: int, round_: int = 1, turn: int = 1) -> TurnCandidate:
...
def test_picks_nearest_deadline() -> None:
    ...
    assert chosen.game_id == "G_0001"


def test_ties_break_by_game_then_round_then_turn() -> None:
    # All share a deadline; order is decided by game_id, then round, then turn.
