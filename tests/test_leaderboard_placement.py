...
def _at(day: int = 4) -> datetime:
    # On/after the 2026-06-03 leaderboard cutoff.
...
async def _seed_completed_match(
    ...
    db.add(match)
    await db.flush()
    for i, (agent_id, wins, score) in enumerate(stats):
        p = await seat_player(db, match.id, agent_id, i=i)
...
async def test_pd_leaderboard_ratings_unchanged() -> None:
    ...
    async with factory() as db:
        ...
        assert [(r.rank, r.display_name, r.rating) for r in standard.rows] == [
            (1, "bot-A", 1512.0),
            (2, "bot-B", 1500.0),
            (3, "bot-C", 1488.0),
        ]
        bonus = (await load_leaderboard_sections(db, rating_mode="bonus", included="agents"))[0]
        assert [(r.rank, r.display_name, round(r.rating, 4)) for r in bonus.rows] == [
            (1, "bot-A", 1514.4),
            (2, "bot-B", 1498.8),
            (3, "bot-C", 1486.8),
        ]
...
async def test_per_game_placement_key_reorders_its_section() -> None:
    ...
    async with factory() as db:
        ...
        assert [(r.rank, r.display_name) for r in section.rows] == [
            (1, "bot-Y"),
            (2, "bot-X"),
        ]
