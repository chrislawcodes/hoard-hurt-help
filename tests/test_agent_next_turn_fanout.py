...
async def _seat_agent_in_active_match_no_turn(
    db: AsyncSession, user: User, connection: Connection, match_id: str
) -> Agent:
    """Seat an AI agent in an ACTIVE match that has NO open turn yet, so a
    long-poll on `next-turn` enters its hold loop instead of serving immediately."""
    now = datetime.now(timezone.utc)
    match = Match(
        id=match_id,
        name=f"match-{match_id}",
...
async def test_long_poll_stops_serving_when_connection_paused_mid_hold(
    ...
    """If the connection is PAUSED while the long-poll is holding, the hold bails
    out the same way — mirroring `require_connection`'s CONNECTION_PAUSED gate.
    A regression that drops the explicit re-check would hold the full window
    (the paused connection is also "dead" for routing, so it serves no turn) —
    asserting a prompt return guards against that."""
    async with session_factory() as db:
        user = await make_user(db)
        connection, key = await make_connection(db, user)
        await _seat_agent_in_active_match_no_turn(db, user, connection, "M_0820")
        await db.commit()
        connection_id = connection.id

    async def pause_connection_and_open_turn() -> None:
        await asyncio.sleep(0.15)
        async with session_factory() as db:
            stored = (
                await db.execute(
    ...
    loop = asyncio.get_event_loop()
    started = loop.time()
    pauser = asyncio.create_task(pause_connection_and_open_turn())
    r = await client.get(
        "/api/agent/next-turn?hold_seconds=5&interval_seconds=0.05",
        headers={"X-Connection-Key": key},
    )
