"""The agent-facing rules text must describe the betrayal sting and stay in
...
def test_rules_text_documents_the_betrayal_sting():
    assert "Betrayal sting" in GAME_RULES_TEXT
    # The sting magnitude shown must match the constant, and differ from base HURT.
...
def test_custom_round_counts_keep_the_sting():
    text = make_game_rules_text(total_rounds=10, turns_per_round=10)
    assert "Betrayal sting" in text
