...
serves the cached copy (no rebuild, even after a newer game finishes), clear
forces a rebuild, and an empty DB falls back to the bundled sample replay. The
stale-while-revalidate behaviour itself is covered in test_swr_cache.py.
"""
...
async def test_hit_within_ttl_serves_cached_copy(reset_db):
    ...
    first = await load_showcase_replay_cached()
    ...
    second = await load_showcase_replay_cached()
...
async def test_clear_forces_rebuild(reset_db):
    ...
    clear_showcase_replay_cache()
    await _seed_showcase_match(reset_db, match_id="M_a", when=_BASE, user_base=100)
    await load_showcase_replay_cached()

    await _seed_showcase_match(reset_db, match_id="M_b", when=_BASE + timedelta(days=1), user_base=200)
    clear_showcase_replay_cache()
    fresh = await load_showcase_replay_cached()
    assert fresh[0] == "M_b"
...
async def test_no_showcase_falls_back_to_sample(reset_db):
    ...
    clear_showcase_replay_cache()
    rc_game_id, rc_data, rc_game_type = await load_showcase_replay_cached()
