...
async def _autouse_reset_db(reset_db: async_sessionmaker) -> None:
    """One test in this file (`test_hard_delete_with_foreign_keys_enforced`)
    drives `client` without requesting `reset_db` by name; this keeps every
    test in the file on a fresh DB, matching this file's old autouse fixture.
    """
...
async def test_archived_bot_cannot_join_new_game(client, reset_db):
    ...
    r = await client.post(
        ...
        data={"agent_id": agent.id, "chosen_provider": "claude", "display_name": "atlas2", "strategy_prompt": ""},
        cookies=_signed_in_cookies(user.id),
        follow_redirects=False,
    )
    assert r.status_code == 404
