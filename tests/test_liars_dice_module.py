...
async def reset_db(monkeypatch: pytest.MonkeyPatch):
    ...
    # Rebind the production globals too: engine code opens its own sessions via
    # app.db.SessionLocal, and without this every xdist worker shares the real
    # file-backed DB (intermittent UNIQUE-constraint failures).
